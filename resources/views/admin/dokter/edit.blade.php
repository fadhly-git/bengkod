<x-layouts.app title="Edit Dokter">
    ...
    <div class="ui-page-head">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="ui-title !text-[1.55rem] md:!text-[1.7rem]">Edit Dokter</h2>
                <p class="ui-subtitle">Perbarui profil dokter dan poli penugasannya.</p>
            </div>
        </div>
    </div>
            ...
            <form action="{{ route('admin.dokter.update', $dokter->id) }}" method="POST">
                ...
                <div class="grid md:grid-cols-2 gap-4 mb-5">
                    <div class="ui-field">
                        ...
                        <input type="text" name="nama" value="{{ old('nama', $dokter->nama) }}" class="ui-input @error('nama') is-error @enderror" required>
                        @error('nama')<p class="ui-error">{{ $message }}</p>@enderror
                    </div>

                    <div class="ui-field">
                        <label class="ui-label">Email <span class="text-red-500">*</span></label>
                        <input type="email" name="email" value="{{ old('email', $dokter->email) }}" class="ui-input @error('email') is-error @enderror" required>
                ...
                <div class="grid md:grid-cols-2 gap-4 mb-5">
                    <div class="ui-field">
                        ...
                        <input type="text" name="no_ktp" value="{{ old('no_ktp', $dokter->no_ktp) }}" class="ui-input @error('no_ktp') is-error @enderror" required>
                        @error('no_ktp')<p class="ui-error">{{ $message }}</p>@enderror
                    </div>

                    <div class="ui-field">
                        <label class="ui-label">No. HP <span class="text-red-500">*</span></label>
                        <input type="text" name="no_hp" value="{{ old('no_hp', $dokter->no_hp) }}" class="ui-input @error('no_hp') is-error @enderror" required>
                ...
                <div class="ui-field mb-5">
                    ...
                    <textarea name="alamat" rows="3" class="ui-textarea @error('alamat') is-error @enderror" required>{{ old('alamat', $dokter->alamat) }}</textarea>
                ...
                <div class="grid md:grid-cols-2 gap-4 mb-6">
                    <div class="ui-field">
                        ...
                        <select name="id_poli" class="ui-input @error('id_poli') is-error @enderror" required>
                            ...
                            @foreach($polis as $poli)
                                <option value="{{ $poli->id }}" @selected(old('id_poli', $dokter->id_poli) == $poli->id)>{{ $poli->nama_poli }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="ui-field">
                        ...
                        <input type="password" name="password" class="ui-input @error('password') is-error @enderror">
                        <p class="ui-help">Kosongkan jika tidak ingin mengganti password.</p>
